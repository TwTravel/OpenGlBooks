...
public:
	...
	CChessBoard *GetBoard() { return m_chessBoard; }
	void OnSelection(float row, float col);

	bool GetBoardHighlight(int row, int col) { return m_chessBoard->GetHighlight(row, col); }
	bool IsValidMove(int pieceIdx, int newRow, int newCol);

	void SetCurrentState(char state) { m_gameState = state; }
	char GetCurrentState() { return m_gameState; }
	char GetCurrentMoveColor() { return m_currentMoveColor; }
};
