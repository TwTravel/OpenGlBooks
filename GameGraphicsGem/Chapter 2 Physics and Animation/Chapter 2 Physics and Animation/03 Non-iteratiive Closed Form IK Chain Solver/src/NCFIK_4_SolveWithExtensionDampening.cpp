...
///////////////////////////////////////////////
// This function takes a scalar parameter and limits it to be less than the limit value
// The 'softening' value modulates the value accross the range limit -> limit - softening.
// Thiis function can be applied to any scalar value, and in this case it is used to limit
// the distance that the ik goal can get from the root of the IK chain. This means that the
// IK chain will not hit the limit value, or snap straight, but will gently slow down 
// as the limit is reached. 
float LimitValue( float val, float limit, float softening )
{
	softening /= ( sinf( DegToRad( 45.0f ) ) * tanf( DegToRad( 22.5f ) ) );

	if( val > ( limit + (softening * tanf( DegToRad( 22.5f ) ) )))
	   return limit;
	if( val < ( limit - (softening * ( sinf( DegToRad( 45.0f ) ) - tanf( DegToRad( 22.5f ) ) ) ) ) )
	   return val;

	return (float)( limit + ( softening * ( cosf( asinf( tanf( DegToRad( 22.5f ) ) + ( ( limit - val ) / softening ) ) ) ) - softening ) );
}
...
void NCFIK_4_SolveWithExtensionDampening( IKChain *pChain, GLVector3f goalPosition, unsigned int options )
{
	...
	vecToIkGoal			= goalPosition - pChain->m_bonePositions[ 0 ];
	distToIkGoal		= vecToIkGoal.length();
	// Limit the extention to 98% and ramp it up over 5% of the chains length
	vecToIkGoal			*= ( LimitValue( distToIkGoal, pChain->m_fChainLength * 0.98f, pChain->m_fChainLength * 0.08f ) ) / distToIkGoal;
	goalPosition		= pChain->m_bonePositions[ 0 ] + vecToIkGoal;
