...
int drawGLScene(void)
{
    ...
    glLoadIdentity();
        ...
        glTranslatef(-Cube->vPosition.x,-Cube->vPosition.y,-Cube->vPosition.z);     // hold camera on model position

        Asix();

        Cube->Render(1); // rendering Model
...
glEnable(GL_BLEND);
    ...
    glScalef(1,0,1);
        glDepthMask(false);
          Cube->Render(0);     // rendering Model Shadow
        glDepthMask(true);
    glPopMatrix();
    ...
    glColor4f(0.4f,0.5f,0.9f,0.40f);
    glBegin(GL_QUADS);
        glVertex3f(-100+Cube->vPosition.x,0,-100+Cube->vPosition.z);
        glVertex3f(-100+Cube->vPosition.x,0, 100+Cube->vPosition.z);
        glVertex3f( 100+Cube->vPosition.x,0, 100+Cube->vPosition.z);
        glVertex3f( 100+Cube->vPosition.x,0,-100+Cube->vPosition.z);
    glEnd();

    int iPointX=(int)(Cube->vPosition.x/5.0)*5;
    int iPointZ=(int)(Cube->vPosition.z/5.0)*5;
