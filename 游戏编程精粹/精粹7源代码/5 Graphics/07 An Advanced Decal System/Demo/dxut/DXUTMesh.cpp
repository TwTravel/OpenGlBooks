...
HRESULT CDXUTMesh::Create( LPDIRECT3DDEVICE9 pd3dDevice, LPCWSTR strFilename )
{
    ...
    if( FAILED( hr = D3DXLoadMeshFromX( strPath, D3DXMESH_MANAGED, pd3dDevice, 
                                        &pAdjacencyBuffer, &pMtrlBuffer, &pShaderBuffer,
    ...
    D3DXEFFECTINSTANCE* pShaderData = (D3DXEFFECTINSTANCE*)pShaderBuffer->GetBufferPointer();
    D3DXMATERIAL* d3dxMtrls = (D3DXMATERIAL*)pMtrlBuffer->GetBufferPointer();
    hr = CreateMaterials( strPath, pd3dDevice, d3dxMtrls, m_dwNumMaterials, pShaderData );
                           ...
                           LPD3DXFILEDATA pFileData )
{
    ...
    D3DXMATERIAL* d3dxMtrls = (D3DXMATERIAL*)pMtrlBuffer->GetBufferPointer();
    hr = CreateMaterials( L"", pd3dDevice, d3dxMtrls, m_dwNumMaterials, NULL );
                           ...
                           D3DXMATERIAL* pd3dxMaterials, DWORD dwMaterials )
{
    ...
    HRESULT hr;
    hr = CreateMaterials( L"", pd3dDevice, pd3dxMaterials, dwMaterials, NULL );
...
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::CreateMaterials( LPCWSTR strPath, IDirect3DDevice9 *pd3dDevice, D3DXMATERIAL* d3dxMtrls, DWORD dwNumMaterials, D3DXEFFECTINSTANCE* pShaderData )
{
    ...
    if( d3dxMtrls && m_dwNumMaterials > 0 )
    {
        ...
        m_pMaterials = new CDXUTMeshMaterial[m_dwNumMaterials];
        ...
        {
            // Copy the material
            m_pMaterials[i].m_d3dMaterial = d3dxMtrls[i].MatD3D;
            m_pTextures[i] = NULL;

            for( DWORD j=0; j<pShaderData[i].NumDefaults; j++ )
            {
                LPD3DXEFFECTDEFAULT param = &pShaderData[i].pDefaults[j];

                if ( param->Type == D3DXEDT_STRING )
                {
                    CDXUTMeshTexture meshTexture;
                    WCHAR strTexture[MAX_PATH];
                    WCHAR strTexturePath[MAX_PATH];

                    MultiByteToWideChar( CP_ACP, 0, (LPCSTR)param->pValue, param->NumBytes, strTexture, param->NumBytes );
                    strTexture[ param->NumBytes ] = 0;

                    wcscpy( strTexturePath, strPath );
                    wcscat( strTexturePath, strTexture );

                    strcpy( meshTexture.m_strName, param->pParamName );
                    meshTexture.m_pTexture = NULL;

                    DXUTGetGlobalResourceCache().CreateTextureFromFile( pd3dDevice, strTexturePath, &meshTexture.m_pTexture );

                    m_pMaterials[i].m_Textures.Add( meshTexture );
                }
            }
...
HRESULT CDXUTMesh::Destroy()
{
    ...
    for( UINT i=0; i<m_dwNumMaterials; i++ )
    {
        ...
        for( int j=0; j<m_pMaterials[i].m_Textures.GetSize(); j++ )
            SAFE_RELEASE( m_pMaterials[i].m_Textures[j].m_pTexture );
    }
                          ...
                          bool bDrawAlphaSubsets )
{
    ...
    if( bDrawOpaqueSubsets )
    {
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            if( m_bUseMaterials )
            {
                if( m_pMaterials[i].m_d3dMaterial.Diffuse.a < 1.0f )
                    continue;
                pd3dDevice->SetMaterial( &m_pMaterials[i].m_d3dMaterial );
                           ...
                           bool bDrawAlphaSubsets )
{
    ...
    if( bDrawOpaqueSubsets )
    {
        ...
        for( UINT p = 0; p < cPasses; ++p )
        {
            ...
            for( DWORD i=0; i<m_dwNumMaterials; i++ )
            {
                CDXUTMeshMaterial * material = &m_pMaterials[i];
                bool bCommitChanges = false;

                if( m_bUseMaterials )
                {
                    if( material->m_d3dMaterial.Diffuse.a < 1.0f )
                        continue;
                    if( hTexture )
                    ...
                    if( hDiffuse )
                        pEffect->SetVector( hDiffuse, (D3DXVECTOR4*)&material->m_d3dMaterial.Diffuse );
                    if( hAmbient )
                        pEffect->SetVector( hAmbient, (D3DXVECTOR4*)&material->m_d3dMaterial.Ambient );
                    if( hSpecular )
                        pEffect->SetVector( hSpecular, (D3DXVECTOR4*)&material->m_d3dMaterial.Specular );
                    if( hEmissive )
                        pEffect->SetVector( hEmissive, (D3DXVECTOR4*)&material->m_d3dMaterial.Emissive );
                    if( hPower )
                        pEffect->SetFloat( hPower, material->m_d3dMaterial.Power );

                    bCommitChanges = true;
                }
                
                if( material->m_Textures.GetSize() )
                {
                    for( int j=0; j<material->m_Textures.GetSize(); j++ )
                    {
                        pEffect->SetTexture( material->m_Textures[j].m_strName, material->m_Textures[j].m_pTexture );
                    }
        ...
        for( UINT p = 0; p < cPasses; ++p )
        {
            ...
            for( DWORD i=0; i<m_dwNumMaterials; i++ )
            {
                if( m_bUseMaterials )
                {
                    ...
                    // No conversion is needed.
                    if( hDiffuse )
                        pEffect->SetVector( hDiffuse, (D3DXVECTOR4*)&m_pMaterials[i].m_d3dMaterial.Diffuse );
                    if( hAmbient )
                        pEffect->SetVector( hAmbient, (D3DXVECTOR4*)&m_pMaterials[i].m_d3dMaterial.Ambient );
                    if( hSpecular )
                        pEffect->SetVector( hSpecular, (D3DXVECTOR4*)&m_pMaterials[i].m_d3dMaterial.Specular );
                    if( hEmissive )
                        pEffect->SetVector( hEmissive, (D3DXVECTOR4*)&m_pMaterials[i].m_d3dMaterial.Emissive );
                    if( hPower )
                        pEffect->SetFloat( hPower, m_pMaterials[i].m_d3dMaterial.Power );
                    pEffect->CommitChanges();
                }
                m_pMesh->DrawSubset( i );
            }
