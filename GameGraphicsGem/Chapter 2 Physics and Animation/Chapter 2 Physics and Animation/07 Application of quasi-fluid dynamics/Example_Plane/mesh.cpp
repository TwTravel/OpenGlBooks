...
vect3 vForceSum=vect3(0,0,0);
 ...
 vMediumSurfPos=-vMediumSurfN*(vPosition^vMediumSurfN); // boundary surface conversion to local coordinates system
	  ...
	  //SET LEFT AILERON POINTS POSITION
	  vVertexPos[76]=vect3(fVertexes_data[76])+vect3(0.f,0.13f,0.f)*fControlParameter[1];
	  vVertexPos[85]=vect3(fVertexes_data[85])+vect3(0.f,0.13f,0.f)*fControlParameter[1];
	  //SET RIGHT AILERON POINTS POSITION
	  vVertexPos[82]=vect3(fVertexes_data[82])+vect3(0.f,-0.13f,0.f)*fControlParameter[1];
	  vVertexPos[89]=vect3(fVertexes_data[89])+vect3(0.f,-0.13f,0.f)*fControlParameter[1];

	  //SET ELEVATOR POINTS POSITION
	  vVertexPos[242]=vect3(fVertexes_data[242])+vect3(0.f,0.13f,0.f)*fControlParameter[2];
	  vVertexPos[252]=vect3(fVertexes_data[252])+vect3(0.f,0.13f,0.f)*fControlParameter[2];
	  vVertexPos[254]=vect3(fVertexes_data[254])+vect3(0.f,0.13f,0.f)*fControlParameter[2];
	  vVertexPos[268]=vect3(fVertexes_data[268])+vect3(0.f,0.13f,0.f)*fControlParameter[2];
	  vVertexPos[274]=vect3(fVertexes_data[274])+vect3(0.f,0.13f,0.f)*fControlParameter[2];


for (int i=0;i<36;i++)   // recalculation only morphed triangles ( where one or more points change position )
{
...
// Triangle cuted by medium boundary ( triangle partly in air and water
    ...
    //!!!!!! Add Engine Force;
	vForceSum+=vect3(-fControlParameter[0]*400.f,0,0); // 400N
