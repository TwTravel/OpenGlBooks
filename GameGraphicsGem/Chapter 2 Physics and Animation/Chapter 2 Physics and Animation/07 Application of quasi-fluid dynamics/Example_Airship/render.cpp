...
int drawGLScene(void)
{
    ...
    glTranslatef(0,0,-5);
	glColor3f(1.0,1,1.0);
 	glRasterPos2f(-1.0f,1.1f);
 	glPrint("key     Control                Velocity: %0.2fkm/h",Airship->vVelocity.l()*3.6);
 	glRasterPos2f(-1.0f,1.0f);
 	glPrint("Q/A    - THRUST    parameter: %0.2f ",fControlParameter[0]);
 	glRasterPos2f(-1.0f,0.9f);
 	glPrint("W/S    - AILERON   parameter: %0.2f ",fControlParameter[1]);
 	glRasterPos2f(-1.0f,0.8f);
 	glPrint("E/D    - RUDDER    parameter: %0.2f ",fControlParameter[2]);

        glTranslatef(X,Y,Z-10); // shift camera
        glRotatef(float(iMy)+90.f,1.f,0.f,0.f);
        glRotatef(float(iMx),0.f,1.f,0.f);

        glTranslatef(-Airship->vPosition.x,-Airship->vPosition.y,-Airship->vPosition.z);     // hold camera on model position

        Asix();

        Airship->Render(1); // rendering Model
...
glEnable(GL_BLEND);
    ...
    glScalef(1,0,1);
        glDepthMask(false);
          Airship->Render(0);     // rendering Model Shadow
        glDepthMask(true);
    glPopMatrix();
    ...
    glColor4f(0.4f,0.5f,0.9f,0.40f);
    glBegin(GL_QUADS);
        glVertex3f(-100+Airship->vPosition.x,0,-100+Airship->vPosition.z);
        glVertex3f(-100+Airship->vPosition.x,0, 100+Airship->vPosition.z);
        glVertex3f( 100+Airship->vPosition.x,0, 100+Airship->vPosition.z);
        glVertex3f( 100+Airship->vPosition.x,0,-100+Airship->vPosition.z);
    glEnd();

    int iPointX=(int)(Airship->vPosition.x/5.0)*5;
    int iPointZ=(int)(Airship->vPosition.z/5.0)*5;
