...
void NCFIK_2_BasicSolve( IKChain *pChain, GLVector3f goalPosition, unsigned int options )
{
	...
	for( unsigned int i = 0; i < pChain->m_iNumBones; i++ )
	{
		...
		if(i <= ( pChain->m_iNumBones - 2 ) )
		{
			...
			{
				maxFkBoneAngle	= acosf( Clamp( ( Sq( pChain->m_fBoneLengths[ i ] ) + Sq( distToFkChainTip ) - Sq( remainingChainLength ))/( 2.0f * pChain->m_fBoneLengths[i] * distToFkChainTip ), -1.0f, 1.0f ));
				maxIkBoneAngle	= acosf( Clamp( ( Sq( pChain->m_fBoneLengths[ i ] ) + Sq( distToIkGoal ) - Sq( remainingChainLength ))/( 2.0f * pChain->m_fBoneLengths[i] * distToIkGoal ), -1.0f, 1.0f ));
				...
				if( options&OPTION_DEBUG )
				{
					...
					// Draw some lines indicating ths max angles
