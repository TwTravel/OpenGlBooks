...
int drawGLScene(void)
{
	...
	glColor3f(1.0,1,1.0);
 	...
 	glPrint("key     Control                Velocity: %0.2fkm/h",Plane->vVelocity.l()*3.6);
 	...
 	glPrint("E/D    - RUDDER    parameter: %0.2f ",fControlParameter[2]);
        ...
        glTranslatef(-Plane->vPosition.x,-Plane->vPosition.y,-Plane->vPosition.z);     // hold camera on model position

        Asix();

        Plane->Render(1); // rendering Model
...
glEnable(GL_BLEND);
    ...
    glScalef(1,0,1);
        glDepthMask(false);
          Plane->Render(0);     // rendering Model Shadow
        glDepthMask(true);
    glPopMatrix();
    ...
    glColor4f(0.4f,0.5f,0.9f,0.40f);
    glBegin(GL_QUADS);
        glVertex3f(-100+Plane->vPosition.x,0,-100+Plane->vPosition.z);
        glVertex3f(-100+Plane->vPosition.x,0, 100+Plane->vPosition.z);
        glVertex3f( 100+Plane->vPosition.x,0, 100+Plane->vPosition.z);
        glVertex3f( 100+Plane->vPosition.x,0,-100+Plane->vPosition.z);
    glEnd();

    int iPointX=(int)(Plane->vPosition.x/5.0)*5;
    int iPointZ=(int)(Plane->vPosition.z/5.0)*5;
